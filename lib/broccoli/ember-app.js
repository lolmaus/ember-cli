...
var logger = require('heimdalljs-logger')('ember-cli:ember-app');
...
EmberApp.prototype._initOptions = function(options) {
  ...
  var vendorTree = existsSync(vendorPath) ? new WatchedDir(vendorPath) : null;
...
EmberApp.prototype._import = function(assetPath, options, directory, subdirectory, extension) {
  ...
  var outputFileExtension =  options.outputFile && path.extname(options.outputFile);

  // if (isType(assetPath, 'js', {registry: this.registry})) {
  if (isType(assetPath, 'js', {registry: this.registry}) || outputFileExtension === '.js') {
    if (options.using) {
      var self = this;
  ...
  } else if (extension === '.css' || outputFileExtension === '.css') {
    if (options.type === 'vendor') {
...
function _lastOneWins(fileList, assetPath, options) {
  var assetIndex = fileList.indexOf(assetPath);
