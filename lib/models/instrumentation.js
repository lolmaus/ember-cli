...
Instrumentation.prototype._buildSummary = function (result, resultAnnotation) {
  ...
  var buildTree = tree.loadFromNode(this.instrumentations.build.node);
  var buildSteps = 0;
  var totalTime = 0;
  ...
  for (nodeItr = buildTree.preOrderIterator();;) {
...
Instrumentation.prototype._instrumentationTreeFor = function(name) {
  var tree = tree.loadFromNode(this.instrumentations[name]node);
  return tree;
}
...
Instrumentation.prototype.start = function (name) {
  ...
  this.instrumentations[name].cookie = cookie;
  this.instrumentations[name].node = this._heimdall.current;
};
