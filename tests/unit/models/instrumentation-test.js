...
describe('models/instrumentation.js', function() {
  ...
  describe('._instrumenationTreeFor', function() {
    ...
    function assertTreeValidAPI(name, tree) {
      var preOrderNames = itr2Array(tree.preOrderIterator()).map(function (x) { return x.label.name; });
      expect(preOrderNames, 'pre order').to.eql([
        name, 'a', 'b1', 'c1', 'b2', 'c2', 'd1', 'c3'
      ]);

      var postOrderNames = itr2Array(tree.postOrderIterator()).map(function (x) { return x.label.name; });
      expect(postOrderNames, 'post order').to.eql([
        'c1', 'b1', 'd1', 'c2', 'c3', 'b2', 'a', name
      ]);

      var c2 = itr2Array(tree.preOrderIterator()).filter(function (x) {
